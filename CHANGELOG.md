...
## [0.2.3](https://github.com/Dark-Alex-17/managarr/compare/v0.2.2...v0.2.3) - 2024-11-26
...
- Populate the GitHub release with the changelog [skip ci]
- version 0.2.2 → 0.3.0 [skip ci]
- Create a new GitHub tag as well when releasing [skip ci]
- version 0.2.2 → 0.3.0 [skip ci]
- Update the release flow [skip ci]
- *(README)* Added Sonarr support to the README [skip ci]
- Fixed the commitizen commit to skip CI [skip ci]
- *(README)* Updated the README to include new features that are available in the Sonarr CLI release
- *(lint)* Added allow dead code directives around certain structs that are causing linter complaints because these will either be used once sonarr UI work begins, or in future Servarr developments that will make life easier
...
- *(README)* Added a key and proper GitHub markdown emoji names instead of direct unicode symbols for the features tables
- *(README)* Updated the readme to use a table of checkmarks to indicate which features are present in the CLI and TUI
- Merge remote-tracking branch 'origin/main' into sonarr-cli-support
- *(linter)* Removed unused imports that were missed on the last commit
- *(models)* Fixed the test for the default series_history value in the sonarr_data
- *(modals)* Removed the unnecessary season_details field from the SeasonDetailsModal
- published the managarr-tui-widget to crates.io so I changed the Cargo.toml to now pull from crates.io instead of directly using the repo for the widget
