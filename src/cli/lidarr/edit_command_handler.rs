...
use crate::{
  ...
  models::{
    ...
    lidarr_models::{EditArtistParams, IndexerSettings, LidarrSerdeable},
    Serdeable,
  },
...
pub enum LidarrEditCommand {
  ...
  Indexer {
    ...
    #[arg(
      long,
      help = "Only use this indexer for artists with at least one matching tag ID. Leave blank to use with all artists.",
      value_parser,
  ...
  Artist {
    ...
    #[arg(
      long,
      help = "Enable monitoring of this artist in Lidarr so Lidarr will automatically download this artist if it is available",
      conflicts_with = "disable_monitoring"
    )]
    enable_monitoring: bool,
    #[arg(
      long,
      help = "Disable monitoring of this artist so Lidarr does not automatically download the artist if it is found to be available",
