...
mod tests {
  ...
  mod cli {
    ...
    #[test]
    fn test_refresh_downloads_has_no_arg_requirements() {
      let result =
        Cli::command().try_get_matches_from(["managarr", "readarr", "refresh", "downloads"]);
  ...
  mod handler {
    ...
    #[tokio::test]
    async fn test_handle_refresh_downloads_command() {
      let mut mock_network = MockNetworkTrait::new();
      mock_network
        .expect_handle_network_event()
        .with(eq::<NetworkEvent>(ReadarrEvent::UpdateDownloads.into()))
        .times(1)
        .returning(|_| {
          Ok(Serdeable::Readarr(ReadarrSerdeable::Value(
            json!({"testResponse": "response"}),
      ...
      let app_arc = Arc::new(Mutex::new(App::test_default()));
      let refresh_command = ReadarrRefreshCommand::Downloads;
