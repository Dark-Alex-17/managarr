...
impl DownloadsHandler<'_, '_> {
  fn extract_download_id(&self) -> i64 {
    self.app.data.readarr_data.downloads.current_selection().id
  }
}

impl<'a, 'b> KeyEventHandler<'a, 'b, ActiveReadarrBlock> for DownloadsHandler<'a, 'b> {
  fn handle(&mut self) {
    let downloads_table_handling_config =
      TableHandlingConfig::new(ActiveReadarrBlock::Downloads.into());

    if !handle_table(
      self,
      |app| &mut app.data.readarr_data.downloads,
  ...
  fn is_ready(&self) -> bool {
    !self.app.is_loading && !self.app.data.readarr_data.downloads.is_empty()
  }
  ...
  fn handle_delete(&mut self) {
    if self.active_readarr_block == ActiveReadarrBlock::Downloads {
      self
        .app
        .push_navigation_stack(ActiveReadarrBlock::DeleteDownloadPrompt.into());
    }
  }

  fn handle_left_right_action(&mut self) {
    match self.active_readarr_block {
      ActiveReadarrBlock::Downloads => handle_change_tab_left_right_keys(self.app, self.key),
      ActiveReadarrBlock::DeleteDownloadPrompt | ActiveReadarrBlock::UpdateDownloadsPrompt => {
  ...
  fn handle_submit(&mut self) {
    match self.active_readarr_block {
      ActiveReadarrBlock::DeleteDownloadPrompt => {
        if self.app.data.readarr_data.prompt_confirm {
          self.app.data.readarr_data.prompt_confirm_action =
            Some(ReadarrEvent::DeleteDownload(self.extract_download_id()));
        }
      ...
      ActiveReadarrBlock::UpdateDownloadsPrompt => {
        if self.app.data.readarr_data.prompt_confirm {
          self.app.data.readarr_data.prompt_confirm_action = Some(ReadarrEvent::UpdateDownloads);
        }
  ...
  fn handle_char_key_event(&mut self) {
    let key = self.key;
    match self.active_readarr_block {
      ActiveReadarrBlock::Downloads => match self.key {
        _ if matches_key!(update, key) => {
          self
            .app
            .push_navigation_stack(ActiveReadarrBlock::UpdateDownloadsPrompt.into());
        }
