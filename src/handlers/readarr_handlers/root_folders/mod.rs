...
use crate::models::servarr_data::readarr::readarr_data::{
  ADD_ROOT_FOLDER_BLOCKS, ADD_ROOT_FOLDER_SELECTION_BLOCKS, ActiveReadarrBlock, ROOT_FOLDERS_BLOCKS,
};
...
impl<'a, 'b> KeyEventHandler<'a, 'b, ActiveReadarrBlock> for RootFoldersHandler<'a, 'b> {
  fn handle(&mut self) {
    ...
    if !handle_table(
      self,
      |app| &mut app.data.readarr_data.root_folders,
  ...
  fn is_ready(&self) -> bool {
    !self.app.is_loading && !self.app.data.readarr_data.root_folders.is_empty()
  }
  ...
  fn handle_delete(&mut self) {
    if self.active_readarr_block == ActiveReadarrBlock::RootFolders {
      self
        .app
        .push_navigation_stack(ActiveReadarrBlock::DeleteRootFolderPrompt.into())
    }
  }

  fn handle_left_right_action(&mut self) {
    match self.active_readarr_block {
      ActiveReadarrBlock::RootFolders => handle_change_tab_left_right_keys(self.app, self.key),
      ActiveReadarrBlock::DeleteRootFolderPrompt => handle_prompt_toggle(self.app, self.key),
      _ => (),
    }
  }

  fn handle_submit(&mut self) {
    if self.active_readarr_block == ActiveReadarrBlock::DeleteRootFolderPrompt {
      if self.app.data.readarr_data.prompt_confirm {
        self.app.data.readarr_data.prompt_confirm_action = Some(ReadarrEvent::DeleteRootFolder(
          self.extract_root_folder_id(),
        ));
      }
  ...
  fn handle_char_key_event(&mut self) {
    let key = self.key;
    match self.active_readarr_block {
      ActiveReadarrBlock::RootFolders => match self.key {
        _ if matches_key!(refresh, key) => {
          self.app.should_refresh = true;
        }
        _ if matches_key!(add, key) => {
          self.app.data.readarr_data.add_root_folder_modal =
            Some(AddReadarrRootFolderModal::from(&self.app.data.readarr_data));
          self.app.data.readarr_data.selected_block =
            BlockSelectionState::new(ADD_ROOT_FOLDER_SELECTION_BLOCKS);
          self
            .app
            .push_navigation_stack(ActiveReadarrBlock::AddRootFolderPrompt.into());
        }
        _ => (),
      },
      ActiveReadarrBlock::DeleteRootFolderPrompt => {
