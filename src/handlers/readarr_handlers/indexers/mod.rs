...
use crate::models::servarr_data::readarr::readarr_data::{
  ActiveReadarrBlock, EDIT_INDEXER_NZB_SELECTION_BLOCKS, EDIT_INDEXER_TORRENT_SELECTION_BLOCKS,
...
impl IndexersHandler<'_, '_> {
  fn extract_indexer_id(&self) -> i64 {
    self.app.data.readarr_data.indexers.current_selection().id
  }
}

impl<'a, 'b> KeyEventHandler<'a, 'b, ActiveReadarrBlock> for IndexersHandler<'a, 'b> {
  fn handle(&mut self) {
    let indexers_table_handling_config =
      TableHandlingConfig::new(ActiveReadarrBlock::Indexers.into());

    if !handle_table(
      self,
      |app| &mut app.data.readarr_data.indexers,
      indexers_table_handling_config,
    ) {
      match self.active_readarr_block {
        _ if EditIndexerHandler::accepts(self.active_readarr_block) => {
          EditIndexerHandler::new(self.key, self.app, self.active_readarr_block, self.context)
            .handle()
        }
        _ if IndexerSettingsHandler::accepts(self.active_readarr_block) => {
          IndexerSettingsHandler::new(self.key, self.app, self.active_readarr_block, self.context)
            .handle()
        }
        _ if TestAllIndexersHandler::accepts(self.active_readarr_block) => {
          TestAllIndexersHandler::new(self.key, self.app, self.active_readarr_block, self.context)
            .handle()
        }
  ...
  fn is_ready(&self) -> bool {
    !self.app.is_loading && !self.app.data.readarr_data.indexers.is_empty()
  }
  ...
  fn handle_delete(&mut self) {
    if self.active_readarr_block == ActiveReadarrBlock::Indexers {
      self
        .app
        .push_navigation_stack(ActiveReadarrBlock::DeleteIndexerPrompt.into());
    }
  }

  fn handle_left_right_action(&mut self) {
    match self.active_readarr_block {
      ActiveReadarrBlock::Indexers => handle_change_tab_left_right_keys(self.app, self.key),
      ActiveReadarrBlock::DeleteIndexerPrompt => handle_prompt_toggle(self.app, self.key),
      _ => (),
    }
  }
  ...
  fn handle_submit(&mut self) {
    match self.active_readarr_block {
      ActiveReadarrBlock::DeleteIndexerPrompt => {
        if self.app.data.readarr_data.prompt_confirm {
          self.app.data.readarr_data.prompt_confirm_action =
            Some(ReadarrEvent::DeleteIndexer(self.extract_indexer_id()));
        }
      ...
      ActiveReadarrBlock::Indexers => {
        self
          .app
          .push_navigation_stack(ActiveReadarrBlock::EditIndexerPrompt.into());
        self.app.data.readarr_data.edit_indexer_modal = Some((&self.app.data.readarr_data).into());
        let protocol = &self
          .app
          .data
          .readarr_data
  ...
  fn handle_esc(&mut self) {
    match self.active_readarr_block {
      ...
      ActiveReadarrBlock::TestIndexer => {
        self.app.pop_navigation_stack();
        self.app.data.readarr_data.indexer_test_errors = None;
      }
  ...
  fn handle_char_key_event(&mut self) {
    let key = self.key;
    match self.active_readarr_block {
      ActiveReadarrBlock::Indexers => match self.key {
        ...
        _ if matches_key!(test, key) => {
          self
            .app
            .push_navigation_stack(ActiveReadarrBlock::TestIndexer.into());
        }
        _ if matches_key!(test_all, key) => {
          self
            .app
            .push_navigation_stack(ActiveReadarrBlock::TestAllIndexers.into());
        }
        _ if matches_key!(settings, key) => {
          self
            .app
            .push_navigation_stack(ActiveReadarrBlock::AllIndexerSettingsPrompt.into());
          self.app.data.readarr_data.selected_block =
