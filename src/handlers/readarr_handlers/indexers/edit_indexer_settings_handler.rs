...
impl<'a, 'b> KeyEventHandler<'a, 'b, ActiveReadarrBlock> for IndexerSettingsHandler<'a, 'b> {
  ...
  fn is_ready(&self) -> bool {
    !self.app.is_loading && self.app.data.readarr_data.indexer_settings.is_some()
  }
  ...
  fn handle_left_right_action(&mut self) {
    if self.active_readarr_block == ActiveReadarrBlock::AllIndexerSettingsPrompt {
  ...
  fn handle_submit(&mut self) {
    match self.active_readarr_block {
          ...
          ActiveReadarrBlock::IndexerSettingsConfirmPrompt => {
            if self.app.data.readarr_data.prompt_confirm {
              self.app.data.readarr_data.prompt_confirm_action = Some(
                ReadarrEvent::EditAllIndexerSettings(self.build_edit_indexer_settings_params()),
              );
              self.app.should_refresh = true;
            } else {
              self.app.data.readarr_data.indexer_settings = None;
            }
      ...
      | ActiveReadarrBlock::IndexerSettingsRetentionInput
      | ActiveReadarrBlock::IndexerSettingsMaximumSizeInput
      | ActiveReadarrBlock::IndexerSettingsRssSyncIntervalInput => self.app.pop_navigation_stack(),
      _ => (),
    }
  }
