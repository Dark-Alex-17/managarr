...
pub(super) struct AlbumDetailsHandler<'a, 'b> {
  key: Key,
  app: &'a mut App<'b>,
...
impl<'a, 'b> KeyEventHandler<'a, 'b, ActiveLidarrBlock> for AlbumDetailsHandler<'a, 'b> {
  fn handle(&mut self) {
    ...
    if !self.handle_tracks_table_events(tracks_table_handling_config)
      && !self.handle_album_history_table_events(album_history_table_handling_config)
      && !self.handle_album_releases_table_events(album_releases_table_handling_config)
    {
  ...
  fn is_ready(&self) -> bool {
    !self.app.is_loading
      && if let Some(album_details_modal) = &self.app.data.lidarr_data.album_details_modal {
      ...
      ActiveLidarrBlock::AutomaticallySearchAlbumPrompt => {
        if self.app.data.lidarr_data.prompt_confirm {
          self.app.data.lidarr_data.prompt_confirm_action = Some(
            LidarrEvent::TriggerAutomaticAlbumSearch(self.extract_artist_id_album_id_tuple()),
          );
        }
  ...
  fn handle_char_key_event(&mut self) {
    let key = self.key;
    match self.active_lidarr_block {
      ActiveLidarrBlock::AlbumDetails if matches_key!(toggle_monitoring, self.key) => {
        self.app.data.lidarr_data.prompt_confirm = true;
        self.app.data.lidarr_data.prompt_confirm_action = Some(
          LidarrEvent::ToggleTrackMonitoring(self.extract_track_id()),
        );
