use super::readarr_ui_utils::create_history_event_details;
use crate::app::App;
...
impl DrawUi for HistoryUi {
  fn accepts(route: Route) -> bool {
    if let Route::Readarr(active_readarr_block, _) = route {
      return HISTORY_BLOCKS.contains(&active_readarr_block);
    }
  ...
  fn draw(f: &mut Frame<'_>, app: &mut App<'_>, area: Rect) {
    if let Route::Readarr(active_readarr_block, _) = app.get_current_route() {
      ...
      if active_readarr_block == ActiveReadarrBlock::HistoryItemDetails {
...
fn draw_history_table(f: &mut Frame<'_>, app: &mut App<'_>, area: Rect) {
  let current_selection = if app.data.readarr_data.history.items.is_empty() {
    ReadarrHistoryItem::default()
  } else {
    app.data.readarr_data.history.current_selection().clone()
  };
  if let Route::Readarr(active_readarr_block, _) = app.get_current_route() {
    ...
    let history_table = ManagarrTable::new(
      Some(&mut app.data.readarr_data.history),
      history_row_mapping,
    )
    .block(layout_block_top_border())
    .loading(app.is_loading)
    .sorting(active_readarr_block == ActiveReadarrBlock::HistorySortPrompt)
    .searching(active_readarr_block == ActiveReadarrBlock::SearchHistory)
    .search_produced_empty_results(active_readarr_block == ActiveReadarrBlock::SearchHistoryError)
    .filtering(active_readarr_block == ActiveReadarrBlock::FilterHistory)
    .filter_produced_empty_results(active_readarr_block == ActiveReadarrBlock::FilterHistoryError)
