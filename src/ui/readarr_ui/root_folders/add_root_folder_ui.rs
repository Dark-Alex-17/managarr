...
struct AddRootFolderPromptHighlights {
  name: bool,
  path: bool,
  monitor: bool,
...
fn add_root_folder_prompt_highlights(
  selected_block: ActiveReadarrBlock,
) -> AddRootFolderPromptHighlights {
  AddRootFolderPromptHighlights {
    name: selected_block == ActiveReadarrBlock::AddRootFolderNameInput,
    path: selected_block == ActiveReadarrBlock::AddRootFolderPathInput,
    monitor: selected_block == ActiveReadarrBlock::AddRootFolderSelectMonitor,
    monitor_new_items: selected_block == ActiveReadarrBlock::AddRootFolderSelectMonitorNewItems,
    quality_profile: selected_block == ActiveReadarrBlock::AddRootFolderSelectQualityProfile,
    metadata_profile: selected_block == ActiveReadarrBlock::AddRootFolderSelectMetadataProfile,
    tags: selected_block == ActiveReadarrBlock::AddRootFolderTagsInput,
    confirm: selected_block == ActiveReadarrBlock::AddRootFolderConfirmPrompt,
  }
}
...
fn draw_add_root_folder_confirmation_prompt(f: &mut Frame<'_>, app: &mut App<'_>, area: Rect) {
  ...
  if let Route::Readarr(active_readarr_block, _) = app.get_current_route() {
    let name_input_box = InputBox::new(&name.text)
      ...
      .selected(active_readarr_block == ActiveReadarrBlock::AddRootFolderNameInput);
    let path_input_box = InputBox::new(&path.text)
      .offset(path.offset.load(Ordering::SeqCst))
      .label("Path")
      .highlighted(highlights.path)
      .selected(active_readarr_block == ActiveReadarrBlock::AddRootFolderPathInput);
    let tags_input_box = InputBox::new(&tags.text)
      .offset(tags.offset.load(Ordering::SeqCst))
      .label("Tags")
      .highlighted(highlights.tags)
      .selected(active_readarr_block == ActiveReadarrBlock::AddRootFolderTagsInput);

    match active_readarr_block {
      ActiveReadarrBlock::AddRootFolderNameInput => name_input_box.show_cursor(f, name_area),
      ActiveReadarrBlock::AddRootFolderPathInput => path_input_box.show_cursor(f, path_area),
      ActiveReadarrBlock::AddRootFolderTagsInput => tags_input_box.show_cursor(f, tags_area),
      _ => (),
    }
