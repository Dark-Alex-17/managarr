...
struct EditIndexerPromptHighlights {
  name: bool,
  url: bool,
  api_key: bool,
  seed_ratio: bool,
...
fn edit_indexer_prompt_highlights(
  selected_block: ActiveReadarrBlock,
) -> EditIndexerPromptHighlights {
  EditIndexerPromptHighlights {
    name: selected_block == ActiveReadarrBlock::EditIndexerNameInput,
    url: selected_block == ActiveReadarrBlock::EditIndexerUrlInput,
    api_key: selected_block == ActiveReadarrBlock::EditIndexerApiKeyInput,
    seed_ratio: selected_block == ActiveReadarrBlock::EditIndexerSeedRatioInput,
    tags: selected_block == ActiveReadarrBlock::EditIndexerTagsInput,
    priority: selected_block == ActiveReadarrBlock::EditIndexerPriorityInput,
    rss: selected_block == ActiveReadarrBlock::EditIndexerToggleEnableRss,
    automatic_search: selected_block == ActiveReadarrBlock::EditIndexerToggleEnableAutomaticSearch,
    interactive_search: selected_block
      == ActiveReadarrBlock::EditIndexerToggleEnableInteractiveSearch,
    confirm: selected_block == ActiveReadarrBlock::EditIndexerConfirmPrompt,
  }
}
...
fn draw_edit_indexer_prompt(f: &mut Frame<'_>, app: &mut App<'_>, area: Rect) {
  ...
  let edit_indexer_modal_option = &app.data.readarr_data.edit_indexer_modal;
  let protocol = &app.data.readarr_data.indexers.current_selection().protocol;
  ...
  if edit_indexer_modal_option.is_some() {
    ...
    if let Route::Readarr(active_readarr_block, _) = app.get_current_route() {
      ...
      let name_input_box = InputBox::new(&edit_indexer_modal.name.text)
        ...
        .selected(active_readarr_block == ActiveReadarrBlock::EditIndexerNameInput);
      let url_input_box = InputBox::new(&edit_indexer_modal.url.text)
        .offset(edit_indexer_modal.url.offset.load(Ordering::SeqCst))
        .label("URL")
        .highlighted(highlights.url)
        .selected(active_readarr_block == ActiveReadarrBlock::EditIndexerUrlInput);
      let api_key_input_box = InputBox::new(&edit_indexer_modal.api_key.text)
        ...
        .label("API Key")
        .highlighted(highlights.api_key)
        .selected(active_readarr_block == ActiveReadarrBlock::EditIndexerApiKeyInput);
      let tags_input_box = InputBox::new(&edit_indexer_modal.tags.text)
        ...
        .selected(active_readarr_block == ActiveReadarrBlock::EditIndexerTagsInput);
      let priority_input_box = InputBox::new(&priority)
        .cursor_after_string(false)
        .label("Indexer Priority ▴▾")
        .highlighted(highlights.priority)
        .selected(active_readarr_block == ActiveReadarrBlock::EditIndexerPriorityInput);
      ...
      if protocol == "torrent" {
        let seed_ratio_input_box = InputBox::new(&edit_indexer_modal.seed_ratio.text)
          ...
          .label("Seed Ratio")
          .highlighted(highlights.seed_ratio)
          .selected(active_readarr_block == ActiveReadarrBlock::EditIndexerSeedRatioInput);
        let tags_input_box = InputBox::new(&edit_indexer_modal.tags.text)
          .offset(edit_indexer_modal.tags.offset.load(Ordering::SeqCst))
          .label("Tags")
          .highlighted(highlights.tags)
      ...
      let auto_search_checkbox = Checkbox::new("Enable Automatic Search")
        ...
        .highlighted(highlights.automatic_search);
      let interactive_search_checkbox = Checkbox::new("Enable Interactive Search")
        ...
        .highlighted(highlights.interactive_search);
