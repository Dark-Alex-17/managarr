...
impl DrawUi for TestAllIndexersUi {
  fn accepts(route: Route) -> bool {
    if let Route::Readarr(active_readarr_block, _) = route {
      return active_readarr_block == ActiveReadarrBlock::TestAllIndexers;
    }
...
fn decorate_test_result_row_with_style<'a>(
  result: &IndexerTestResultModalItem,
  row: Row<'a>,
) -> Row<'a> {
  if result.is_valid {
    row.success()
...
fn draw_test_all_indexers_test_results(f: &mut Frame<'_>, app: &mut App<'_>, area: Rect) {
  let is_loading = app.is_loading || app.data.readarr_data.indexer_test_all_results.is_none();
  let current_selection = if let Some(test_all_results) =
    app.data.readarr_data.indexer_test_all_results.as_ref()
  ...
  let test_results_row_mapping = |result: &IndexerTestResultModalItem| {
    ...
    decorate_test_result_row_with_style(result, row)
  };

  let indexers_test_results_table = ManagarrTable::new(
    app.data.readarr_data.indexer_test_all_results.as_mut(),
